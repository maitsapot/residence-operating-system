...
def create_issue(db: Session, payload: IssueCreate):
...
def get_issues(
    ...
    *,
    ...
    if not include_archived:
        query = query.filter(Issue.archived_at.is_(None))
    if status:
    ...
    if space_id:
        query = query.filter(Issue.space_id == space_id)
...
def get_issues_by_space(
    ...
    *,
    ...
    query = db.query(Issue).filter(Issue.space_id == space_id)
    if not include_archived:
        query = query.filter(Issue.archived_at.is_(None))
    if status:
        query = query.filter(Issue.status == status)
...
def archive_issue(db: Session, issue_id: str):
    issue = get_issue(db, issue_id)
    if issue.archived_at:
        return issue

    issue.archived_at = func.now()
    db.commit()
    db.refresh(issue)
    return issue


def restore_issue(db: Session, issue_id: str):
    issue = get_issue(db, issue_id)
...
def assign_issue(db: Session, issue_id: str, user_id: str, updated_by: str):
...
def get_issue_updates(db: Session, issue_id: str):
